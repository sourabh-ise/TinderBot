...
const ProfileSelector = ({profile, onSwipe}) => (profile ? (
...
const MatchesList = ({matches, onSelectMatch}) => {
    ...
    return (<div className="rounded-lg shadow-lg p-4">
        ...
        <ul>
            {matches.map((match, index) => (<li key={index} className="mb-2">
                    <button
...
const ChatScreen = ({currentMatch, conversation, refreshState}) => {
    ...
    return currentMatch ? (<div className='rounded-lg shadow-lg p-4'>
            ...
            <div className="h-[50vh] border rounded-lg overflow-y-auto mb-6 p-4 bg-gray-50">
                {conversation.messages.map((message, index) => (<div key={index}
                        ...
                        <div
                            ...
                            {message.authorId === 'user' ? (<User size={15}/>) : (<img
                                src={`http://localhost:8080/images/${currentMatch.imageUrl}`}
                                className="w-11 h-11 rounded-full"
                            />)}
                            <div
                                className={`max-w-xs px-4 py-2 rounded-2xl ${message.authorId === 'user' ? 'bg-blue-500 text-white ml-2' : 'bg-gray-200 text-gray-800 mr-2'}`}
                            >
                                {message.messageText}
                            </div>
                        </div>
